...
describe('RegisterUseCase', () => {
  it('creates a tenant and owner user successfully', async () => {
    ...
    // The id is generated by the use case, not by the column default: the RLS
    // policy on `tenants` is `id = app.tenant_id`, so the context has to be set
    // before the INSERT (audit finding H1).
