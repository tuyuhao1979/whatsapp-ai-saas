...
so this service could not create a collection and every document failed to
index, while the flow engine could never find one. Nothing covered the pair: the
verification stack had no vector store, and the Python CI ran unit tests with no
server to talk to.
...
_PYPROJECT = _REPO_ROOT / "services" / "rag-indexer" / "pyproject.toml"
